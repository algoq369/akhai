 ...
 * - The BoT paper reports 88% cost reduction vs ToT and +11% on complex reasoning (arXiv:2406.04271)
...
function parseDistillationResult(
  ...
  // Tokens saved = original buffer size minus the distilled output size (measured, not assumed)
  const tokensSaved = Math.max(0, totalTokens - estimateTokens(summary + keyInsights.join(' ')));
