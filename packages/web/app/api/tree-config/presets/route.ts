...
export async function POST(request: NextRequest) {
  try {
    ...
    const sessionId = request.cookies.get('akhai_session')?.value || null;
    if (!sessionId) {
      return NextResponse.json({ error: 'Session required to save presets' }, { status: 401 });
    }
    ...
    const result = stmt.run(null, sessionId, name, description || '', JSON.stringify(weights));
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const sessionId = request.cookies.get('akhai_session')?.value || null;
    if (!sessionId) {
      return NextResponse.json({ error: 'Session required to delete presets' }, { status: 401 });
    }
