...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      db.exec('ALTER TABLE users ADD COLUMN x_username TEXT');
    } catch {}
    try {
    ...
    if (!user) {
      ...
      db.prepare(
        'INSERT OR IGNORE INTO users (id, x_username, x_id, created_at) VALUES (?, ?, ?, ?)'
      ).run(userId, xUsername, xId, new Date().toISOString());
