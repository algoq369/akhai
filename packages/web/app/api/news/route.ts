...
function categorize(headline: string): string {
  ...
  if (lower.match(/agi\b|asi\b|superintelligen|general intelligence|singularity|consciousness/))
    return 'agi';
  if (lower.match(/crypto|bitcoin|btc|eth|blockchain|defi|token|nft/)) return 'crypto';
  if (
    lower.match(
      /ai\b|artificial|llm|gpt|claude|gemini|openai|anthropic|deepseek|model|neural|frontier/
    )
  )
    return 'ai';
      ...
      items: [
        {
          category: 'ai',
          source: 'akhai',
          headline: 'Sovereign intelligence engine active',
          url: '/',
          age: 'now',
        },
        {
          category: 'crypto',
          source: 'akhai',
          headline: 'Multi-chain analysis ready',
          url: '/',
          age: 'now',
        },
        {
          category: 'tech',
          source: 'akhai',
          headline: 'Grounding guard protecting against hallucination',
          url: '/',
          age: 'now',
        },
      ],
