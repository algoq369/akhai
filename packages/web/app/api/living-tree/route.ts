...
function determineDominantLaw(hermeticData: {
  ...
  const dominant = laws.reduce((max, law) => (law.text.length > max.text.length ? law : max));
...
function determineOverallVibration(
  ...
  const vibrationScores = topics.map((t) => {
    const score = t.vibration_level === 'high' ? 3 : t.vibration_level === 'medium' ? 2 : 1;
    return score * t.importance_score;
  });

  const avgScore = vibrationScores.reduce((sum, s) => sum + s, 0) / vibrationScores.length;
