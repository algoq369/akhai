...
/**
 * KetherState - Tracks the sovereignty boundary for each query
 *
...
export interface KetherState {
  ...
  /** Sephirothic level (1=Malkuth/factual, 10=Kether/meta-cognitive) */
  ascentLevel: number
 ...
 * These are Qliphothic (hollow) and must be purified.
 */
 ...
 * activateKether
 *
 * Activates the Kether Protocol for a given query.
 * This is the first function called in the query processing pipeline.
 *
 * @param query - The user's query
 * @returns KetherState with sovereignty boundaries established
 */
export function activateKether(query: string): KetherState {
 ...
 *
 * Determines the Sephirothic level of the query.
 * Higher levels require greater sovereignty awareness.
 *
 * 1 (Malkuth) = Simple facts
 * 5 (Tiferet) = Integration/synthesis
 * 10 (Kether) = Meta-cognitive/consciousness
 *
 * @param query - User's query
 ...
 * @param state - KetherState from activateKether
 * @returns true if boundaries respected, false if violated
 */
export function checkSovereignty(response: string, state: KetherState): boolean {
 ...
 *
 * @param state - KetherState
 * @returns Footer text or null
 */
export function generateSovereigntyFooter(state: KetherState): string | null {
 ...
 * getKetherMetadata
 *
 * Extracts metadata about Kether state for logging/analytics.
 *
 * @param state - KetherState
 * @returns Metadata object
 */
export function getKetherMetadata(state: KetherState) {
  return {
    ...
    ketherActivatedAt: state.activatedAt.toISOString(),
  }
}
