...
import type { Layer } from './layer-registry'
...
export const KABBALISTIC_TERMS = {
  ...
  kether: {
    ...
    hebrew: 'כֶּתֶר',
    meaning: 'Crown',
    fullMeaning: 'The highest Layer representing meta-cognitive awareness and divine will',
  ...
  daat: {
    ...
    hebrew: 'דַּעַת',
    meaning: 'Knowledge',
    fullMeaning: 'Hidden knowledge, the non-Layer that connects all',
...
/**
 * Auto-format a string containing Layer origin names
 * Replaces bare Layer origin names with explained versions
 *
 * Example: "Kether → Malkuth" becomes "Kether (Crown) → Malkuth (Kingdom)"
 */
export function explainLayerPath(path: string): string {
  const layerNames = [
 ...
 */
export function getAllLayers(): KabbalisticTermKey[] {
  return [
