/**
 * Antipattern Report Formatter
 *
 * Formats AntipatternReport into human-readable markdown annotations.
 */

import type { AntipatternReport, ShadowActivation, WeightRecommendation } from './types'

/**
 * Health status icons
 */
const HEALTH_ICONS: Record<AntipatternReport['overallHealth'], string> = {
  balanced: '◉',
  minor_imbalance: '◐',
 ...
 */
const HEALTH_LABELS: Record<AntipatternReport['overallHealth'], string> = {
  balanced: 'Balanced',
...
/**
 * Format a complete annotation
 */
export function formatGnosticAnnotation(report: AntipatternReport): string {
 ...
 */
export function formatCompactAnnotation(report: AntipatternReport): string {
 ...
 */
function formatHeader(report: AntipatternReport): string {
  const icon = HEALTH_ICONS[report.overallHealth]
  const label = HEALTH_LABELS[report.overallHealth]
 ...
 */
function formatProcessingPath(path: AntipatternReport['processingPath']): string {
...
| Aspect | Value |
|--------|-------|
| Dominant Layer | **${path.dominantSephirah}** |
 ...
 */
function formatGuardAnalysis(analysis: AntipatternReport['guardAnalysis']): string {
...
function formatShadowActivations(shadows: ShadowActivation[]): string {
  ...
  shadows.forEach((shadow) => {
    ...
    lines.push(`#### ${shadow.antipattern} (${shadow.sephirah} Shadow)`)
...
function formatRecommendations(recommendations: WeightRecommendation[]): string {
  ...
  recommendations.forEach((rec, i) => {
    ...
    lines.push(`   - Adjust Layer ${rec.adjustWeight.sephirah}: ${delta}`)
    lines.push('')
  })
 ...
 */
export function formatConsoleAnnotation(report: AntipatternReport): string {
  const icon = HEALTH_ICONS[report.overallHealth]
  const label = HEALTH_LABELS[report.overallHealth]
  ...
  if (report.shadowsActivated.length > 0) {
    ...
    report.shadowsActivated.forEach((s) => {
      output += `  Warning: ${s.antipattern}: ${Math.round(s.intensity * 100)}%\n`
    })
  }
 ...
 */
export function formatJSONAnnotation(report: AntipatternReport): object {
  return {
