...
function extractKeyPhrases(text: string): string[] {
  ...
  const nounPhrasePattern =
    /\b(?:[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2}|(?:artificial intelligence|machine learning|deep learning|neural network|quantum computing|climate change|cryptocurrency|blockchain technology|renewable energy|gene therapy|space exploration))\b/g;
...
function detectDomain(query: string, response: string) {
  ...
  return {
    // Technology domains
    isAI: /artificial intelligence|machine learning|neural network|deep learning|llm|gpt|claude|ai model/i.test(
      combined
    ),
    ...
    isClimatetech: /climate|carbon|emission|renewable energy|solar|wind|sustainability/i.test(
      combined
    ),
    isBiotech: /gene therapy|crispr|biotech|genomics|pharmaceutical|drug development/i.test(
      combined
    ),
    isAerospace: /space|nasa|rocket|satellite|mars|aerospace/i.test(combined),

    // Geopolitical/economic
    isGeopolitics: /geopolitic|sanction|trade war|china|russia|us\s+policy|export control/i.test(
      combined
    ),
...
export function generateIntelligentLinks(
  ...
  if (domain.isCrypto) {
    const cryptoSymbol = mainTopic.match(/bitcoin|btc/i)
      ? 'bitcoin'
      : mainTopic.match(/ethereum|eth/i)
        ? 'ethereum'
        : mainTopic.match(/solana|sol/i)
          ? 'solana'
  ...
  if (annotations && annotations.length > 0) {
    annotations.slice(0, 2).forEach((ann) => {
      ...
      if (hasMetric && domain.isCrypto) {
        links.push({
          id: `coinmetrics-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`,
