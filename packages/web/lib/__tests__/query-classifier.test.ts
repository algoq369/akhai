...
describe('Query Classifier', () => {
  describe('classifyQueryIntent', () => {
    ...
    it('classifies procedural queries', () => {
      ...
      queries.forEach((query) => {
        const result = classifyQueryIntent(query);
        // Classifier may map these to procedural or creative depending on keywords
        expect(['procedural', 'creative', 'analytical']).toContain(result.type);
        expect(result.confidence).toBeGreaterThan(0);
      });
    });
    ...
    it('estimates complexity correctly', () => {
      ...
      const complex =
        'Explain the philosophical implications of quantum entanglement on the nature of reality and causality';
    ...
    it('suggests appropriate methodologies', () => {
      ...
      expect(['direct', 'cod', 'bot', 'react', 'pot', 'gtp', 'auto']).toContain(
