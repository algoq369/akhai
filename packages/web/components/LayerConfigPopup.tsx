...
/**
 * Layer Configuration Popup
 *
 * Small popup modal for configuring individual Layer weight.
...
export function LayerConfigPopup({
  ...
  useEffect(() => {
    if (layerNodeLevel && weights[layerNodeLevel] !== undefined) {
      setTempValue(Math.round(weights[layerNodeLevel] * 100))
    }
  }, [layerNodeLevel, weights])

  if (!layerNodeLevel) return null
  const meta = LAYER_METADATA[layerNodeLevel as Layer]
  if (!meta) return null

  const handleSave = () => {
    setWeight(layerNodeLevel, tempValue / 100)
    onClose()
  }
