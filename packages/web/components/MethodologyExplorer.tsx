          ...
          >
            ...
            {METHODOLOGY_DETAILS.map((method, index) => (
              ...
              >
                ...
                <div className="text-[10px] font-mono text-relic-slate mt-1">{method.name}</div>
              </button>
            ))}
          </div>
        ) : (
          ...
          <div className="w-[160px] bg-relic-white/95 dark:bg-relic-void/95 backdrop-blur-md border border-relic-mist dark:border-relic-slate/30 shadow-2xl animate-fade-in">
            ...
            <div className="border-b border-relic-mist/30 dark:border-relic-slate/20 px-1.5 py-1 flex items-center justify-between">
              <div className="flex items-center gap-1">
                <div className="text-sm text-relic-slate dark:text-white">
                  {selectedDetail?.symbol}
                </div>
                <h2 className="text-[10px] font-mono text-relic-slate dark:text-white">
                  {selectedDetail?.name}
                </h2>
              </div>
              <button
                onClick={handleBack}
                className="text-[8px] text-relic-silver dark:text-relic-ghost hover:text-relic-slate dark:hover:text-white"
              >
                ✕
              </button>
            </div>
            ...
            <div className="px-1.5 py-1 space-y-1">
              {/* Single step */}
              <p className="text-[6px] text-relic-slate dark:text-relic-ghost leading-tight">
                {selectedDetail?.howItWorks[0]}
              </p>
              ...
              <div className="pt-0.5 border-t border-relic-mist/20 dark:border-relic-slate/10 space-y-0.5">
                <div className="flex justify-between">
                  ...
                  <span className="text-[6px] font-mono text-relic-slate dark:text-white">
                    {selectedDetail?.metrics.tokens}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[6px] text-relic-silver dark:text-relic-ghost">lat</span>
                  <span className="text-[6px] font-mono text-relic-slate dark:text-white">
                    {selectedDetail?.metrics.latency}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[6px] text-relic-silver dark:text-relic-ghost">$</span>
                  <span className="text-[6px] font-mono text-relic-slate dark:text-white">
                    {selectedDetail?.metrics.cost}
                  </span>
                </div>
              </div>
            </div>
