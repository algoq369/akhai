...
import { treePositions, treePaths } from './LayerTreeFull.constants';
        ...
        {showPaths &&
          pathConnections.map((path, index) => {
            ...
            if (path.fromMeta.pillar === path.toMeta.pillar && path.fromMeta.pillar === 'left') {
              ...
              path.fromMeta.pillar === path.toMeta.pillar &&
              path.fromMeta.pillar === 'right'
            ) {
              strokeColor = '#3b82f6'; // Blue for Mercy pillar
            } else if (
              path.fromMeta.pillar === path.toMeta.pillar &&
              path.fromMeta.pillar === 'middle'
            ) {
            ...
            const curveOffset =
              Math.abs(toPos.x - fromPos.x) > Math.abs(toPos.y - fromPos.y) ? 15 : 0;
