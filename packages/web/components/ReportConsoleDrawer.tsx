 ...
 * - Layers Tree: Full interactive Tree of Life visualization
  ...
  return (
    <div className="relative flex-shrink-0">
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-white dark:bg-slate-800 border-t-2 border-slate-200 dark:border-slate-700 shadow-sm min-h-[48px]">
        ...
        <button
          onClick={() => setActiveTab(activeTab === 'layers' ? 'closed' : 'layers')}
          className={`px-3 py-1.5 text-[10px] font-mono uppercase tracking-wider rounded transition-colors border ${
            activeTab === 'layers'
...
function MethodologyTab({ currentMethodology, onMethodologyChange }: MethodologyTabProps) {
  const methodologies = [
    ...
    { id: 'cod', symbol: '⋯', name: 'Draft & Refine', description: 'Iterative draft-and-refine process' },
...
function InsightsTab({ analysisMetadata }: InsightsTabProps) {
  ...
  return (
    <div className="p-6 space-y-4">
      {/* Antipattern Guard Shield */}
      {analysisMetadata.antipatternGuardShield && (
        <div>
      ...
      {/* Ascent Tracker */}
      {analysisMetadata.progressState && (
        <div>
          <h4 className="text-xs font-mono uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-2">
            Ascent Progress
          </h4>
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-slate-200 dark:bg-slate-700 rounded-full h-2">
              <div
                ...
                style={{ width: `${(analysisMetadata.progressState.currentLevel / 11) * 100}%` }}
              />
            </div>
            <span className="text-xs font-mono text-slate-600 dark:text-slate-400">
              Level {analysisMetadata.progressState.currentLevel}/11
            </span>
          </div>
        </div>
      )}
      ...
      {/* Synthesis Insights */}
      {analysisMetadata.synthesisInsights && (
        <div>
          <h4 className="text-xs font-mono uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-2">
            Emergent Insights
          </h4>
