 ...
 * Vertical hierarchical layout: top (Kether) to bottom (Malkuth)
 */
const NODE_POSITIONS: Record<number, { x: number; y: number }> = {
  10: { x: 200, y: 30 },    // Kether (Crown) - Top center
  8: { x: 100, y: 120 },    // Binah (Understanding) - Left supernal
  9: { x: 300, y: 120 },    // Chokmah (Wisdom) - Right supernal
  11: { x: 200, y: 200 },   // Da'at (Knowledge) - Center hidden
  6: { x: 100, y: 280 },    // Gevurah (Severity) - Left ethical
  7: { x: 300, y: 280 },    // Chesed (Mercy) - Right ethical
  5: { x: 200, y: 360 },    // Tiferet (Beauty) - Center heart
  3: { x: 100, y: 440 },    // Hod (Glory) - Left astral
  4: { x: 300, y: 440 },    // Netzach (Victory) - Right astral
  2: { x: 200, y: 520 },    // Yesod (Foundation) - Center foundation
  1: { x: 200, y: 600 },    // Malkuth (Kingdom) - Bottom center
}

/**
 * Tree paths connecting Sephiroth
 * Hierarchical parent → child connections (16 paths)
 * Format: [from, to]
 */
const TREE_PATHS: [number, number][] = [
  // Kether → Supernal Triad
  [10, 8],  // Kether → Binah (left)
  [10, 9],  // Kether → Chokmah (right)
  ...
  // Da'at → Ethical Triad
  [11, 6],  // Da'at → Gevurah (left)
  [11, 7],  // Da'at → Chesed (right)

  // Ethical Triad → Tiferet
  [6, 5],   // Gevurah → Tiferet
  [7, 5],   // Chesed → Tiferet

  // Tiferet → Astral Triad
  [5, 3],   // Tiferet → Hod (left)
  [5, 4],   // Tiferet → Netzach (right)
...
const PILLAR_COLORS: Record<string, string> = {
  right: '#94a3b8',    // Mercy (Chokmah, Chesed, Netzach)
  left: '#64748b',     // Severity (Binah, Gevurah, Hod)
  middle: '#8b5cf6',   // Balance (Kether, Tiferet, Yesod, Malkuth, Da'at)
}

function getPillarColor(sefirah: number): string {
  if ([9, 7, 4].includes(sefirah)) return PILLAR_COLORS.right
  if ([8, 6, 3].includes(sefirah)) return PILLAR_COLORS.left
 ...
 * Simple Sefirot Tree Component
 */
export function SimpleSefirotTree() {
  const { weights, setWeight, applyPreset, processingMode, setProcessingMode, activePreset } = useSefirotStore()

  // Preset configurations
  const presets = [
    { name: 'Balanced', weights: Object.fromEntries(Object.keys(SEPHIROTH_METADATA).map(k => [k, 0.5])) },
    {
      name: 'Analytical',
      weights: Object.fromEntries(
        Object.entries(SEPHIROTH_METADATA).map(([k, v]) => [k, v.level <= 5 ? 0.8 : 0.3])
      )
    },
    {
      name: 'Creative',
    ...
    },
    {
      name: 'Compassionate',
      weights: Object.fromEntries(
        Object.entries(SEPHIROTH_METADATA).map(([k, v]) =>
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Configure Sephiroth weights (0-100%) to control AI reasoning layers
        </p>
      </div>
      ...
      <div className="relative w-full" style={{ paddingBottom: '162.5%' }}>
        ...
        >
          ...
          {/* Sephiroth Nodes */}
          {Object.values(SEPHIROTH_METADATA).map(meta => {
      ...
      <div className="text-[9px] text-slate-500 dark:text-slate-400 text-center leading-relaxed px-4">
        Configure Sephirothic weights (0-100%) to control how each AI layer analyzes your queries.
