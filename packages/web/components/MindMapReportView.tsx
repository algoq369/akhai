 ...
 *
 * Note: Layers Tree and Hermetic Lenses consoles are now in the
...
export default function MindMapGrimoireView({ userId, selectedTopics, onTopicExpand }: MindMapGrimoireViewProps) {
  ...
  const handleCreateGrimoire = () => {
    const { weights } = useLayerStore.getState()
    ...
    const layersQuery = `&layers=${encodeURIComponent(JSON.stringify(weights))}`
    const lensesQuery = `&lenses=${encodeURIComponent(JSON.stringify(settings.instinctConfig.activeLenses))}`

    router.push(`/grimoires${topicsQuery}${layersQuery}${lensesQuery}`)
  }
      ...
      <div className="p-4">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <div>
              <h2 className="text-lg font-mono text-relic-void dark:text-white">Report Integration</h2>
