...
export function TopicExpansionPanel({
  ...
  const loadSuggestions = async () => {
    ...
    try {
      ...
      setInsights(
        data.insights.map((text: string) => ({ text, confidence: data.confidence || 0.8 }))
      );
      ...
      >
        ...
        >
          ...
          <div className="flex-none bg-white dark:bg-relic-void border-b border-relic-mist dark:border-relic-slate/30 px-4 py-3">
            ...
            <h3 className="text-sm font-mono text-relic-void dark:text-white mt-2">{topicName}</h3>
            {category && (
                ...
                {insights.map((insight, i) => (
                  <div key={i} className="flex items-start gap-2 text-[9px]">
                    ...
                    <div className="flex-1">
                      <p className="text-relic-void dark:text-white leading-snug">{insight.text}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="px-4 py-3 border-b border-relic-mist dark:border-relic-slate/30">
                ...
                <div className="ml-4 space-y-1">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[9px] text-relic-silver">{metrics.lastActivity}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="px-4 py-3">
              ...
              <div className="ml-4 space-y-2 mb-2">
                {chatMessages.length === 0 && (
                  <div className="text-[9px] text-relic-silver">Ask about {topicName}...</div>
                )}
                {chatMessages.map((msg) => (
                  <div key={msg.id} className="text-[9px] leading-snug">
                    <span className="text-relic-silver">{msg.role === 'user' ? 'you' : 'ai'}:</span>
                    <p className="text-relic-void dark:text-white ml-2 mt-0.5">{msg.content}</p>
                  </div>
                ))}
                {isChatLoading && (
                  <div className="flex gap-1 ml-2">
                    <div
                      className="w-1 h-1 bg-relic-slate rounded-full animate-bounce"
