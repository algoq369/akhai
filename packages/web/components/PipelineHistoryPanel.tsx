...
export default function PipelineHistoryPanel({
  ...
  return (
    <>
      <button
        onClick={onToggle}
        className={`fixed right-0 top-1/2 -translate-y-1/2 z-40 flex flex-col items-center gap-0.5 px-0.5 py-3 rounded-l-md shadow-sm border border-r-0 transition-all duration-200 ${
          isOpen
            ? 'bg-relic-void text-relic-ghost border-relic-slate/60'
            : 'bg-white dark:bg-relic-void/80 text-zinc-400 dark:text-relic-slate/60 border-zinc-200 dark:border-relic-slate/20 hover:text-relic-slate dark:hover:text-relic-silver'
        }`}
      >
        ...
        {groups.length > 0 && (
          <span
            className={`text-[7px] font-mono ${isOpen ? 'text-emerald-400' : 'text-relic-silver/40'}`}
          >
            {groups.length}
          </span>
        )}
          ...
          >
            <div className="flex-shrink-0 px-4 pt-4 pb-3 border-b border-relic-mist/20 dark:border-relic-slate/15">
              <div className="flex items-center justify-between">
                <span className="text-[11px] uppercase tracking-widest text-relic-slate dark:text-relic-silver/80">
                  AI Reasoning
                </span>
                <button
                  onClick={onToggle}
                  className="text-relic-silver/40 hover:text-relic-slate text-[11px] p-1"
                >
                  ✕
                </button>
              </div>
              {groups.length > 0 && (
                <div className="mt-2 grid grid-cols-3 gap-2 text-[9px]">
                  {[
                    ['Queries', String(groups.length)],
                    ['Tokens', totalTokens.toLocaleString()],
                    ['Cost', `$${totalCost.toFixed(4)}`],
                  ].map(([l, v]) => (
                    <div
                      key={l}
                      className="bg-relic-ghost/50 dark:bg-relic-slate/10 rounded px-2 py-1.5 text-center"
                    >
                      <div className="text-[10px] text-relic-slate dark:text-relic-silver/90 font-medium">
                        {v}
                      </div>
                      <div className="text-[7px] text-relic-silver/40 uppercase tracking-wider mt-0.5">
                        {l}
                      </div>
                    </div>
                  ))}
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-relic-slate dark:text-relic-silver/80 leading-snug line-clamp-2">
              {g.queryText || (
                <span className="text-relic-silver/30 italic">query unavailable</span>
              )}
            </p>
            ...
            <div className="flex items-center gap-1.5 mt-1 text-[7px] text-relic-silver/30">
              ...
              {g.duration != null && (
                <>
                  <span>·</span>
                  <span>{formatDuration(g.duration)}</span>
                </>
              )}
              {g.cost > 0 && (
                <>
          ...
          <span
            className={`w-1.5 h-1.5 rounded-full shrink-0 mt-1 ${g.isError ? 'bg-red-500' : g.isComplete ? 'bg-emerald-500/70' : 'bg-amber-400 animate-pulse'}`}
          />
        </div>
      </button>
            ...
            <div className="px-4 pb-4 space-y-2.5">
              ...
              {g.methScores.length > 1 && (
                <div className="bg-relic-ghost/30 dark:bg-relic-slate/5 rounded-md px-3 py-2.5">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-[7px] uppercase tracking-widest text-amber-400/70">
                      Method Comparison
                    </span>
                  </div>
                  <div className="space-y-1.5">
                    {g.methScores.map((m: any) => (
                      <div key={m.method} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-1 bg-relic-slate/8 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${m.method === g.method ? 'bg-amber-400/60' : 'bg-relic-silver/15'}`}
                            style={{ width: `${m.score}%` }}
                          />
                        </div>
                        <span
                          className={`text-[7px] w-6 text-right ${m.method === g.method ? 'text-amber-400/60' : 'text-relic-silver/20'}`}
                        >
                          {m.score}%
                        </span>
                      </div>
              ...
              {g.allLayers.length > 0 && (
                <div className="bg-relic-ghost/30 dark:bg-relic-slate/5 rounded-md px-3 py-2.5">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-[7px] uppercase tracking-widest text-purple-400/70">
                      AI Layers
                    </span>
                    {g.dominantLayer && (
                      <span className="text-[7px] text-purple-400/50 ml-auto">
                        {g.dominantLayer} dominant
                      </span>
                    )}
                  </div>
                    ...
                    {g.allLayers.slice(0, 7).map((l: any) => (
                      <div key={l.name}>
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-[7px] w-[55px] truncate ${l.activated ? 'text-purple-400/80' : 'text-relic-silver/25'}`}
                          >
                            {l.name}
                          </span>
                          <div className="flex-1 h-1 bg-relic-slate/8 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${l.activated ? 'bg-purple-400/50' : 'bg-relic-silver/10'}`}
                              style={{ width: `${Math.min(l.weight, 100)}%` }}
                            />
                          </div>
                          <span
                            className={`text-[7px] w-6 text-right ${l.activated ? 'text-purple-400/50' : 'text-relic-silver/15'}`}
                          >
                            {Math.round(l.weight)}%
                          </span>
                        </div>
                        {l.activated && l.keywords?.length > 0 && (
                          <p className="text-[6px] text-purple-300/30 pl-[63px] mt-0.5">
                            {l.keywords.join(', ')}
                          </p>
                        )}
                      </div>
              ...
              <div className="bg-relic-ghost/30 dark:bg-relic-slate/5 rounded-md px-3 py-2">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[7px] uppercase tracking-widest text-emerald-400/70">
                    Guard
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className={`text-[8px] ${g.guardVerdict === 'pass' ? 'text-emerald-500' : 'text-amber-500'}`}
                  >
                    {g.guardVerdict === 'pass' ? '✓ passed' : `⚠ ${g.guardVerdict}`}
                  </span>
                  {g.guardRisk > 0 && (
                    <span className="text-[7px] text-relic-silver/25">
                      risk {Math.round(g.guardRisk * 100)}%
                    </span>
                  )}
                </div>
                {g.guardReasons?.length > 0 && (
                  <p className="text-[7px] text-relic-silver/35 mt-1">
                    {g.guardReasons.join(' · ')}
                  </p>
                )}
              </div>
              ...
              {g.activeLenses?.length > 0 && (
                <div className="flex items-center gap-1.5 px-1">
                  ...
                  {g.activeLenses.map((l: string) => (
                    <span
                      key={l}
                      className="text-[7px] text-cyan-400/50 bg-cyan-400/5 px-1.5 py-0.5 rounded"
                    >
                      {l}
                    </span>
                  ))}
              ...
              <div className="pt-1.5 border-t border-relic-mist/10 dark:border-relic-slate/8">
                {g.model && (
                  <div className="flex items-center gap-2 text-[7px] text-relic-silver/25 mb-1.5">
                    ...
                    {g.processingMode && (
                      <span className="text-relic-silver/15">· {g.processingMode}</span>
                    )}
                  </div>
                )}
                <div className="grid grid-cols-3 gap-1.5 text-center">
                  {[
                    ['In', g.tokens.input],
                    ['Out', g.tokens.output],
                    ['Total', g.tokens.total],
                  ].map(([l, v]) => (
                    <div
                      key={l as string}
                      className="bg-relic-ghost/20 dark:bg-relic-slate/8 rounded px-1.5 py-1"
                    >
                      <div className="text-[8px] text-relic-slate dark:text-relic-silver/60">
