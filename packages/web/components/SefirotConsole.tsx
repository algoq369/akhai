...
const SEFIRAH_SYMBOLS: Record<number, string> = {
  [Sefirah.MALKUTH]: '⊕', [Sefirah.YESOD]: '◐', [Sefirah.HOD]: '⬡',
  [Sefirah.NETZACH]: '◉', [Sefirah.TIFERET]: '✡', [Sefirah.GEVURAH]: '⚗',
  [Sefirah.CHESED]: '◯', [Sefirah.BINAH]: '⊙', [Sefirah.CHOKMAH]: '☿',
  [Sefirah.KETHER]: '◈', [Sefirah.DAAT]: '◬'
}

const SEFIRAH_COLORS: Record<number, string> = {
  [Sefirah.MALKUTH]: '#92400e', [Sefirah.YESOD]: '#94a3b8', [Sefirah.HOD]: '#eab308',
  [Sefirah.NETZACH]: '#f97316', [Sefirah.TIFERET]: '#22c55e', [Sefirah.GEVURAH]: '#dc2626',
  [Sefirah.CHESED]: '#06b6d4', [Sefirah.BINAH]: '#3b82f6', [Sefirah.CHOKMAH]: '#4f46e5',
  [Sefirah.KETHER]: '#9333ea', [Sefirah.DAAT]: '#06b6d4'
}
...
interface SefirotConsoleProps {
  isOpen?: boolean
  onToggle?: () => void
  onConfigChange?: (config: { sephiroth: Record<number, number>; processingMode?: ProcessingMode }) => void
}

export default function SefirotConsole({ isOpen, onToggle, onConfigChange }: SefirotConsoleProps) {
  const {
  ...
  useEffect(() => {
    onConfigChange?.({ sephiroth: weights, processingMode })
  ...
  const validateAndSave = async () => {
    ...
    try {
      await fetch('/api/tree-config', {
        ...
        body: JSON.stringify({ name: activePreset || 'Custom', sephirothWeights: weights, processingMode, isActive: true })
      })
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 mb-2">
              {Object.values(SEPHIROTH_METADATA).map((s) => {
                const w = weights[s.level] || 0.5
                const pct = Math.round(w * 100)
                return (
                  <div key={s.level} className="flex items-center gap-2">
                    <span className="text-[10px]" style={{ color: SEFIRAH_COLORS[s.level] }}>
                      {SEFIRAH_SYMBOLS[s.level]}
                    </span>
