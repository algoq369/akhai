...
import { XMarkIcon, CheckCircleIcon, ExclamationCircleIcon, ClipboardDocumentIcon, ClipboardDocumentCheckIcon } from '@heroicons/react/24/outline'
...
type PaymentState = 'selecting' | 'pending' | 'confirming' | 'success' | 'failed' | 'expired'
...
export default function CryptoPaymentModal({
  ...
  // Create payment
  const handleCreatePayment = async (currency: string) => {
    setLoading(true)
    setError(null)
            ...
            {paymentState === 'selecting' && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {FEATURED_CURRENCIES.map((curr) => (
                    <button
                      key={curr.id}
                      onClick={() => {
                        setSelectedCurrency(curr.id)
                        handleCreatePayment(curr.id)
                      }}
                      disabled={loading}
