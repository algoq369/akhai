        ...
        <>
          ...
          >
                ...
                return (
                  <div key={layer}>
                    <div className="flex items-baseline gap-1 mb-0">
                      <span
                        className="uppercase tracking-[0.06em] text-[10px] min-w-[70px]"
                        style={{ color }}
                      >
                        {label}
                      </span>
                    ...
                    <div className="flex justify-between mt-0">
                      <span className="text-zinc-400 text-[9px]">{left}</span>
                      <span className="text-zinc-400 text-[9px]">{right}</span>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              {PRESETS.map(({ key, label }) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    import('@/lib/analytics').then(({ trackLayerPresetApplied }) =>
                      trackLayerPresetApplied(key)
                    );
                  }}
                  className="transition-colors text-[10px] uppercase tracking-[0.06em]"
                  style={{
              ...
              {archetype && (
                <span
                  className="text-[10px] tracking-[0.06em] font-semibold transition-colors"
                  style={{
            ...
            <button
              onClick={() => {
                window.location.href = '/tree-of-life?mode=advanced';
              }}
              className="text-[10px] text-zinc-400 hover:text-zinc-500 transition-colors uppercase tracking-[0.06em]"
            >
              advanced &rarr;
            </button>
          </motion.div>
