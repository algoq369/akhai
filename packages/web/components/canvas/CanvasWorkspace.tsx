...
type NodeType = 'query' | 'topic' | 'note' | 'config' | 'stat' | 'diagram' | 'chart' | 'drawing' | 'table' | 'timeline' | 'radar'
...
function generateLocalTable(query: string, response: string): any {
  ...
  const rows = topics.map((t, i) => ({ entity: t, relevance: Math.round(60 + Math.random() * 40) + '%', category: ['Primary', 'Secondary', 'Tertiary'][i % 3] }))
...
function generateLocalTimeline(query: string, response: string): any {
  ...
  const events = topics.map((t, i) => ({ label: t, description: '', color: TOPIC_COLORS[i % TOPIC_COLORS.length] }))
...
function TableRenderer({ data }: { data: any }) {
  ...
  return (
    <div style={{ padding: '8px 10px', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ fontSize: 10, fontWeight: 600, color: '#1e293b', marginBottom: 6, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', wordBreak: 'break-word' }}>{data.title || 'Table'}</div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 8, fontFamily: "'JetBrains Mono',monospace" }}>
          <thead>
            <tr>{cols.map((c: string, i: number) => (
              <th key={i} style={{ padding: '3px 6px', textAlign: 'left', background: '#f59e0b15', color: '#92400e', fontWeight: 600, fontSize: 7, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #f59e0b30' }}>{c}</th>
            ))}</tr>
          </thead>
          <tbody>
        ...
        {events.map((ev: any, i: number) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x} y={above ? 18 : 64} textAnchor="middle" fontSize={8} fontWeight={500} fill="#475569" fontFamily="'JetBrains Mono',monospace">
...
function RadarRenderer({ data }: { data: any }) {
  ...
  return (
    <div style={{ padding: '8px 10px', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ fontSize: 10, fontWeight: 600, color: '#1e293b', marginBottom: 6, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', wordBreak: 'break-word' }}>{data.title || 'Radar'}</div>
      <svg width="100%" height="100%" viewBox="0 0 300 250" preserveAspectRatio="xMidYMid meet" style={{ flex: 1 }}>
        {/* Guide rings */}
        {rings.map(r => (
          <circle key={r} cx={cx} cy={cy} r={(r / 100) * maxR} fill="none" stroke="#f1f5f9" strokeWidth={0.5} />
        ))}
        {/* Axis lines + labels */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '5px 14px', borderBottom: '1px solid #f1f5f9', background: 'white', gap: 4, flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        {selectedIsQuery && (
          <>
            ...
            <button onClick={() => handleGenerate('table')} disabled={!!generating} style={{
              fontSize: 9, padding: '3px 8px', borderRadius: 3, border: '1px solid #f59e0b',
              background: generating === 'table' ? '#f59e0b15' : 'transparent',
              color: '#f59e0b', cursor: generating ? 'wait' : 'pointer', fontFamily: 'inherit',
            }}>{generating === 'table' ? '◌ generating...' : '⊞ table'}</button>
            <button onClick={() => handleGenerate('timeline')} disabled={!!generating} style={{
              fontSize: 9, padding: '3px 8px', borderRadius: 3, border: '1px solid #0ea5e9',
              background: generating === 'timeline' ? '#0ea5e915' : 'transparent',
              color: '#0ea5e9', cursor: generating ? 'wait' : 'pointer', fontFamily: 'inherit',
            }}>{generating === 'timeline' ? '◌ generating...' : '◉ timeline'}</button>
            <button onClick={() => handleGenerate('radar')} disabled={!!generating} style={{
              fontSize: 9, padding: '3px 8px', borderRadius: 3, border: '1px solid #ec4899',
              background: generating === 'radar' ? '#ec489915' : 'transparent',
              color: '#ec4899', cursor: generating ? 'wait' : 'pointer', fontFamily: 'inherit',
            }}>{generating === 'radar' ? '◌ generating...' : '⬡ radar'}</button>
          </>
        )}
